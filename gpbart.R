...
update_phi <- function(x, current_tree_iter,residuals,
  ...
  # Selecting the terminal nodes
  terminal_nodes <- current_tree_iter[names(which(vapply(current_tree_iter, "[[", numeric(1), "terminal") == 1))]
  
  # Selecting the new terminal nodes
  terminal_nodes_phi_proposal <- tree_from_phi_proposal[names(which(vapply(tree_from_phi_proposal, "[[", numeric(1), "terminal") == 1))]
  ...
  g_variance_old <- mapply(terminal_nodes,
                           FUN = function(nodes){
                             nodes$Omega_matrix - crossprod(nodes$Omega_matrix, crossprod(nodes$Omega_plus_I_inv, nodes$Omega_matrix))
                           }, SIMPLIFY = FALSE)
  ...
  log_likelihood_old <- mapply(terminal_nodes, residuals_terminal_nodes,
                               FUN = function(nodes, residuals){
                                 mvtnorm::dmvnorm(x = residuals,
                                                  mean = rep(nodes$mu, nrow(nodes$Omega_matrix)),
                                                  sigma = nodes$Omega_matrix + diag(1/nodes$tau, 
                                                                                    nrow = nrow(nodes$Omega_matrix)), log = TRUE)
                               })
...
tree_count_terminals <- function(gpbart_model) {
  ...
  for(k in seq_along(gpbart_model$trees)) {
    ...
    for(i in seq_len(gpbart_model$number_trees)) {
      tree <- tree_iter[[i]]
      tree_depth[k, i] <- lsum(vapply(tree, "[[", numeric(1), "terminal"))
    }
  }
    return(tree_depth)
}
  ...
  for(m in seq_along(multiple_trees)) {
    ...
    for(i in seq_along(list_nodes)) {
      ...
      if(new_tree[[list_nodes[[i]]]]$terminal == 1 && length(new_tree[[list_nodes[[i]]]]$test_index) > 0) {
        ...
        if(isFALSE(pred_bart_only) && get_cov_star) {
...
predict.gpbart_GPBART <- function(rBart_model, x_test, type = c("all", "mean", "median"),
  ...
  out <- list(
    ...
    sd = switch(type,
                ...
                median = median(1/vapply(rBart_model$tau_store, sqrt, numeric(1)))
    )
  )
...
gpbart_split_mean <- function(mod_gpbart) {
  ...
  all_tree_splits <- array(NA,dim = c(mod_gpbart$number_trees, 50, mod_gpbart$store_size),
                           dimnames = list(paste0("tree_", seq_len(mod_gpbart$number_trees)),
                                           paste0("node_", seq_len(5)),
                                           paste0("iter_", seq_len(mod_gpbart$store_size))))
  
  # Iterating over MH 
  for(k in seq_len(gpbart_mod$store_size)) {
    tree_iter <- mod_gpbart$trees[[k]]
